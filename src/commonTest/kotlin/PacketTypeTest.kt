...
class PacketTypeTest {
    ...
    fun SendPlayerData_can_be_converted() {
        ...
        val bytes = Packet.toByteArray(packetType, playerData)
        assertEquals(19, bytes.size)
        val rowPacket = Packet.fromByteArray(bytes)
        assertNotNull(rowPacket)
        assertEquals(4, rowPacket.id)
        assertEquals(17, rowPacket.array.size)
        val packet = rowPacket.toPacket(packetType)
        assertNotNull(packet)
        assertEquals(4, packet.type.id)
        val packetData = packet.data
        assertEquals(playerData, packetData)
    }
}
